...
describe('AppEventBridge', () => {
  ...
  it('registers the single bridge, validates all payloads and cleans up every listener', () => {
    ...
    expect([...callbacks.keys()]).toEqual(['room:status', 'live:event', 'recording:progress', 'update:status'])
    ...
    callbacks.get('update:status')?.({
      version: 1, state: 'ready', currentVersion: '0.2.0', availableVersion: '0.2.1',
      totalBytes: 100, downloadedBytes: 100, installBlocked: false,
    })
    expect(useUpdateStore.getState().status?.availableVersion).toBe('0.2.1')
    ...
    callbacks.get('update:status')?.({ installerPath: 'C:\\secret\\installer.exe' })
    ...
    expect(LogError).toHaveBeenNthCalledWith(4, 'UI_CONTRACT_INVALID: update:status payload')
    expect(vi.mocked(LogError).mock.calls.flat().join(' ')).not.toContain('secret')

    view.unmount()
    expect([...offs.values()]).toHaveLength(4)
