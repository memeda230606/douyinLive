...
- 任务状态：实现与基础设施、0.2.0 人工引导、0.2.2 stable、0.2.3 canary 及 0.2.2→0.2.3 真实自动升级已完成；剩余受控故障矩阵、24 小时观察和 0.2.3 stable 提升仍为 `IN_PROGRESS`
...
0.2.3 canary、录制目录引导与系统代理修复：

- 源提交/tag：`71af70e86cd2e119b94f3a5e307e6b0621f84cc1` / `v0.2.3`。该版本纳入 settings schema v4 录制目录首次引导、新录制运行时目录切换，以及更新器“进程环境代理优先、Windows 用户静态代理兜底”；源码不写死代理地址，也不修改系统代理；
- Go 全量 test/vet/build、前端 11 文件 39 测试/typecheck/build、dirty=false 正式发布门禁（251 组件、447 个扫描文件）和正式 NSIS 安装矩阵 7/7 通过；
- 安装器 98,404,195 字节，SHA-256 `cf3cfba87f3f817075ac07419f7af4a006effdbddc5bbb522f698b61584fb9a7`；主 EXE 48,821,760 字节，SHA-256 `e6fb27b9a2173a56f7e99aae3823bac43ce2ec3abcfc0bb53dbce6b0c7d1d02a`；发布清单 SHA-256 `7c7d9fec2e462b0d58c31baab2c5c0dc4ce49fe87602d848d8f3980e9dc3740b`；
- canary 信封 1,492 字节，SHA-256 `338e4eb4b1cf5d854f8c1421c45f8db959461df2638d42ffc76245c1d7689d0e`。匿名 HTTPS 回读解析为 0.2.3、提交 `71af70e` 和同一安装器大小/hash；stable 同时回读仍为 0.2.2/提交 `4e51f52`，没有提前提升；
- 0.2.2 不含本次代理修复，首次仅切 canary 且不注入环境代理时仍无法下载，复现了用户的 `UPDATE_CHECK_FAILED`。随后仅对桥接启动进程临时设置 `HTTP_PROXY`/`HTTPS_PROXY`/本机 `NO_PROXY`，自动检查后完整下载安装器，下载文件大小/hash 与正式发布物逐字节一致；
- 用户确认安装后，0.2.2→0.2.3 自动升级成功。正式键与兼容键均为 0.2.3，已安装 EXE hash 与正式发布物一致，新健康标记已写入，settings v3→v4 且 `recordingDirectoryConfirmed=true`，程序目录更新备份和更新助手进程均为 0；
- 再次冷启动 0.2.3 时未注入代理环境变量，机器/用户持久 `HTTP_PROXY` 均为空；客户端通过启用的 Windows 用户静态代理访问 canary，界面显示“已是最新版本 / 当前版本 0.2.3”，没有错误码。临时 `HKLM\SOFTWARE\DouyinLive\Updater\Channel=canary` 已完整移除，主机恢复默认 stable。
...
- 权威 Windows 主机从 2026-07-23 15:42 +08:00 起连续 24 小时稳定观察；只有观察与故障矩阵全部通过后，才能为 0.2.3 签发 stable 信封并提升。
