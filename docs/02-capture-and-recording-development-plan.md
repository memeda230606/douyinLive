...
internal/capture/
  ...
  recording_progress.go      # <=1 Hz 录制进度、跨 attempt 单调聚合与 fencing
internal/app/
...
internal/eventstore/
  ...
  realtime.go                # SQLite 提交后的 100 条/100 ms 隐私安全 UI 批次
```
...
- `live:event` 只发布 SQLite 事务真实提交且未被短窗、数据库或同批去重的 source 事件；每场次最多 100 条或 100 ms 一批，输入上限 2,000 条、发布队列上限 20 批。UI 投递是可丢失副本，慢回调不能反压、重复或改变耐久事件事实。
- `LiveEventBatchDTO` 只包含事件 ID、序列、角色、类型、时间、脱敏显示名、受限内容/数值和解析状态；平台标识、用户哈希、method、dedupe/raw 引用、normalized JSON、protobuf、绝对路径和流 URL 均不跨 Wails 边界。
- `recording:progress` 仅在 `recording/reconnecting` 状态发布，每个当前场次最多 1 Hz。跨 recorder attempt 的时长、字节、分片与重启数单调累积；所有整数饱和在 JavaScript 安全上限，非有限或负值拒绝发布。
- 录制进度以 watcher generation、recorder instance/attempt、场次 `sessionId` 和当前 `operationId` 多重 fencing；进入 Finalize 时先取消 watcher，停止/排空超时或旧回调不能向新场次发布。
- `room:status` 的全局单调 `revision` 是房间、录制错误、重试与恢复告警的唯一有序状态源；前端由其中的 `recordingStatus/retryAt/errorCode/message` 派生告警，不另建 `recording:error`，避免重复和乱序。
- 前端全局事件桥只注册一次，严格 schema 后写入 Zustand；事件按 ID 去重并保留最近 2,000 条，进度只接受精确 `roomId/sessionId/operationId`，状态按 revision 拒绝旧值。UI 副本淘汰不影响 SQLite、checkpoint 或恢复。
...
- `live:event` 只在 SQLite 提交后发布，数据库/同批/缓存重复不会二次进入 UI；100 条/100 ms 批次、2,000 输入上限、20 批发布上限以及阻塞 publisher/关闭 fence 均有重点并发回归。
