...
> 实施状态（2026-07-22）：P4-ACC-001 已完成跨段回放、分析、ASR 降级、隐私导出与真实 GUI 一体化验收；PHASE-4 关闭，项目总进度 90%，下一任务为 P5-ENG-001。
...
P4-ASR 已在分析页独立读取 strict v1 能力状态：默认 disabled 与已选 provider 未配置均显示“主播话术尚未启用”，provider 验证失败显示“暂时不可用”，两者都明确基础互动指标、峰谷和高光继续可用；状态尚在加载时不误报，ready 时隐藏降级提示和转写依赖模块。前端不接收端点、模型、凭据、媒体路径、音频摘要或 provider 原始错误。

P4-EXP 已在分析报告底部提供“CSV / JSON 报告包”面板：默认明确排除昵称、弹幕/转写正文、平台标识、媒体路径、摘要、Cookie、流地址和签名；正文必须通过独立复选框显式选择。导出中显示忙状态、稳定错误和完成后的不透明目录名/文件数，不把绝对路径交给 WebView。strict Zod v1 合同固定五文件顺序、媒体类型、行数、大小与 SHA-256，并拒绝额外字段。
...
### 11.6 P4 一体化验收（2026-07-22）

- 真实交互式 Wails/WebView2 在同一次隔离冷启动中完成历史场次、两段 H.264/AAC 解码、相邻分片自动续播、统一时间轴、分析、ASR disabled 降级和隐私导出；严格结果 9/9 为真。
- 播放器只对时间上相邻且已验证的 MP4 自动续播，遇到真实媒体缺口停在当前分片末尾，不用跨段跳转掩盖缺口。
- `PrintWindow(PW_RENDERFULLCONTENT)` 截图为 1440×900，SHA-256 `c6d4d4dcc632e79a4365303608b3bc46a4b208de4f7e52c23afe33ba6326c05e`；人工复核分析页标题、场次选择、降级说明、四项概要和趋势区均清晰可见、无遮挡。
- 前端共 10 个文件 36 项 Vitest、typecheck、生产构建通过；验收任务、应用进程和隔离运行根均为零残留。完整证据见[P4 一体化验收记录](validation/2026-07-22-p4-phase-acceptance.md)。
