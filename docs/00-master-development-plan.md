...
```yaml
schema_version: 1
updated_at: "2026-07-22T18:25:00+08:00"
authoritative_workspace: "GJS-20250801EFK:D:\douyinLive"
last_verified_branch: "main"
last_verified_head: "5fe0a53e1dbe8c832a40c71557618725661ea738"
project_status: "IN_PROGRESS"
overall_completion_percent: 96
current_phase: "PHASE-5-RELEASE"
current_task: "P5-STB-001"
next_task: "P5-ACC-001"
expected_dirty_paths: []
blockers: []
last_completed_task: "P5-WIN-001"
last_completion_evidence: "新增 fail-closed Windows x64 NSIS 安装器、离线数据库回滚工具、安装/回滚说明和 GitHub 安装矩阵；隔离矩阵 fresh install、覆盖升级、默认卸载保留数据、二次确认清理、WebView2 缺失共 6/6 通过，v5→v6→v5 数据库回滚及外部路径、sidecar、损坏备份反例通过；全量 Go test/vet/build、前端 36 测试/typecheck/build 与 installer matrix 通过。"
resume_instruction: "执行 P5-STB-001：完成 60 分钟多房间等待/模拟开播稳定性、每分钟资源趋势及数据库忙、磁盘满、网络和强制退出恢复门禁。"
```
...
| PHASE-5 发布与稳定性 | 10% | `IN_PROGRESS` | 60% | 6/10 点完成；发布工程、安装升级卸载、WebView2 和数据库回滚矩阵通过 | 60 分钟稳定性与最终发布验收通过 |
| **总体** | **100%** | **`IN_PROGRESS`** | **96%** | PHASE-0 至 PHASE-4 已关闭；P5 发布工程与 Windows 安装升级门禁完成 | 执行 P5-STB-001 60 分钟稳定性门禁 |
...
| P5-STB-001 | 完成 60 分钟稳定性与发布故障门禁 | 2 | P5-WIN-001 | `READY` | 安装升级卸载与数据库回滚矩阵已完成 | 采样资源并覆盖数据库忙、磁盘满、网络和强制退出恢复 |
