...
expected_dirty_paths:
  ...
  - "docs/privacy.md"
  - "docs/user-guide.md"
  - "docs/validation/2026-07-23-p6-oss-auto-update.md"
...
blockers:
  - "P6-UPD-002 需要在 P6 实现提交后生成干净精确 v0.2.0 正式产物，并由用户完成人工引导安装。"
  - "P6-UPD-003 必须基于更高版本 0.2.1 完成真实 canary 升级/失败恢复和 24 小时稳定观察，不能用单元测试替代。"
last_completed_task: "P6-UPD-001"
last_completion_evidence: "OSS/RAM/DPAPI 基础设施、Ed25519 单对象信封、严格客户端、Range/ETag 下载、Wails/React UI、独立更新助手与发布器完成；Go 全量 test/vet/build、前端 37 项、production Wails、0.2.0 脏树候选发布门禁、NSIS 矩阵 7/7 和 OSS 匿名权限/版本恢复实网门禁通过。"
resume_instruction: "先完成 P6 实现提交与干净 v0.2.0 正式构建，交付用户人工安装；随后另建 0.2.1 提交/tag，发布 canary 并执行真实升级、回滚与 24 小时观察，证据齐全后才提升 stable。"
```
...
| PHASE-6 OSS 自动更新 | 5% | `IN_PROGRESS` | 67% | 8/12 点完成；协议、客户端、助手、发布工具、OSS/RAM 与 UI 完成 | 0.2.0 人工安装后发布 0.2.1 canary，完成真实升级/回滚与 24 小时观察后提升 stable |
| **总体** | **100%** | **`IN_PROGRESS`** | **98%** | 0.1.x 全部完成；P6 自动更新实现与基础设施完成 | 完成 0.2.0→0.2.1 真实发布闭环 |
...
| P6-UPD-002 | 生成并人工安装 0.2.0 自动更新引导版 | 2 | P6-UPD-001 | `READY` | 脏树 0.2.0 候选门禁通过；尚无干净精确 tag 正式产物和人工安装证据 | 提交 P6、创建 v0.2.0、正式构建并交付人工安装 |
| P6-UPD-003 | 0.2.1 canary 真实升级、回滚、24 小时观察与 stable 提升 | 2 | P6-UPD-002 | `NOT_STARTED` | 必须使用真实 OSS 与已安装 0.2.0，不以合成测试冒充 | 发布更高版本 canary，完成矩阵和 24 小时观察后提升同一信封 |

阶段任务点按当前阶段统计：P1 共 12 点且已完成；P2 共 20 点且已完成；P3 共 30 点且已完成；P4 共 20 点且已完成；P5 共 11 点且已完成；P6 共 12 点、已完成 8 点。上表若新增或调整点数，必须同步修正本句和对应阶段完成度。P0 的 5 点只用于记录文档基线。
...
- 以 0.2.0 人工引导版和 0.2.1 canary 完成真实升级/回滚及 24 小时观察，再提升同一信封到 stable。

**交付物**：OSS/RAM 基础设施、更新协议与服务、更新助手、发布工具、设置/UI、发布与验收记录。

**退出标准**：0.2.0→0.2.1 真实 canary 升级、活动录制拒绝、断网续传、坏签名/坏包拒绝、安装失败恢复和 24 小时稳定观察全部通过；stable 指针指向完全相同的已验证信封。
