...
expected_dirty_paths:
  ...
  - "docs/validation/2026-07-22-p4-analysis-export.md"
  - "frontend/src/features/analysis/"
  - "frontend/src/generated/wailsjs/go/"
  - "frontend/src/styles/index.css"
  - "internal/analysis/export.go"
  - "internal/analysis/export_test.go"
  - "internal/analysis/service.go"
  - "internal/app/infrastructure.go"
blockers: []
last_completed_task: "P4-EXP-001"
last_completion_evidence: "实现 analysis-export/v1 稳定 manifest、事件/指标桶/转写/媒体分片四类 UTF-8 BOM CSV、UTC+毫秒偏移、逐文件 SHA-256 与 os.Root 限界原子发布；默认排除正文和全部平台/路径/摘要/凭据字段，正文仅显式选择后导出并防表格公式注入。React 使用 strict v1 allowlist 展示隐私说明、显式正文开关和无绝对路径结果。导出专项 20 轮、桌面/应用适配器 10 轮、前端 35 测试、全量 Go test/vet/build、前端与 Wails production build、diff-check 均通过。"
resume_instruction: "执行 P4-ACC-001：完成回放、分析、ASR 降级与导出的一体化验收并关闭 PHASE-4。"
```
...
| PHASE-4 回放与基础分析 | 20% | `IN_PROGRESS` | 90% | P4-PLY-001、P4-ANA-001、P4-ASR-001 与 P4-EXP-001 已完成（18/20 点）：安全回放、版本化分析、ASR 降级和隐私导出可用 | 执行阶段一体化验收 |
| PHASE-5 发布与稳定性 | 10% | `NOT_STARTED` | 0% | — | 发布门禁、安装升级和 60 分钟稳定性通过 |
| **总体** | **100%** | **`IN_PROGRESS`** | **88%** | PHASE-0 至 PHASE-3 已关闭；P4 回放与基础分析完成 18/20 点 | 执行 P4-ACC-001 一体化验收 |
...
| P4-ACC-001 | 完成回放、分析、ASR 降级与导出验收 | 2 | P4-ASR-001、P4-EXP-001 | `READY` | P4 四项实现任务均已关闭，具备一体化验收条件 | 执行回放误差、报告重复、ASR 降级、导出与 GUI 验收 |
