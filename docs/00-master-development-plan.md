...
```yaml
schema_version: 1
updated_at: "2026-07-23T15:45:00+08:00"
authoritative_workspace: "GJS-20250801EFK:D:\douyinLive"
last_verified_branch: "main"
last_verified_head: "71af70e86cd2e119b94f3a5e307e6b0621f84cc1"
...
blockers:
  - "0.2.3 canary 与 0.2.2→0.2.3 真实升级已通过；P6-UPD-003 仍须完成活动录制拒绝、断网续传、坏签名/坏安装包、安装失败自动恢复的受控验收，以及截至 2026-07-24 15:42 +08:00 的 24 小时稳定观察，之后才能提升 stable。"
last_completed_task: "P2-SET-002"
last_completion_evidence: "提交 4697d9e 完成全新安装首次录制目录强制确认、Windows 系统目录选择器、schema v4 兼容迁移和新录制运行时切换；Go 全量 test/vet/build、前端 39 测试/typecheck/build、双次 Wails 与 251 组件/439 文件 release verify 门禁通过。"
resume_instruction: "0.2.3 已发布到 canary，现有 0.2.2 使用一次性进程代理完成真实自动升级；升级后的 0.2.3 在无 HTTP_PROXY/HTTPS_PROXY 环境变量时通过 Windows 用户静态代理检查 canary 并显示已是最新版本，stable 仍为 0.2.2。继续执行剩余受控故障矩阵并观察至 2026-07-24 15:42 +08:00，全部通过后签发 stable 信封并提升。"
```
...
| PHASE-2 Wails 桌面壳与房间管理 | 20% | `DONE` | 100% | 桌面壳、数据基础、房间设置、首次录制目录引导、监控状态机、基础页面及真实 GUI 验收完成 | 下一候选版复验全新安装目录引导；平台或 WebView2 行为变化时复验 |
...
| PHASE-5 发布与稳定性 | 5% | `DONE` | 100% | 11/11 点完成；WebView2 一键安装增强与最终安装包通过验证 | 平台、依赖或公开发行范围变化时复验 |
| PHASE-6 OSS 自动更新 | 5% | `IN_PROGRESS` | 83% | 10/12 点完成；0.2.3 canary、真实升级和 Windows 系统代理兼容已通过 | 完成剩余故障矩阵与 24 小时观察后提升 stable |
| **总体** | **100%** | **`IN_PROGRESS`** | **99%** | 0.1.x 全部完成；P6 canary 真实升级闭环完成 | 完成故障矩阵、观察期与 stable 提升 |

完成度解释：0–10% 为规划与技术准备，11–30% 为采集和桌面基础，31–60% 为录制主链路，61–80% 为回放分析，81–99% 为发布加固，100% 仅在当前明确交付范围的全部门禁通过后填写。

### 0.4 当前任务队列

任务按表格顺序执行；如需拆分新任务，使用同一阶段前缀并更新任务点。当前阶段 PHASE-6 总任务点为 12。
...
| P2-SET-002 | 首次启动录制目录引导与运行时切换 | 0 | P3-MEDIA-001 | `DONE` | [验证记录](validation/2026-07-23-recording-directory-onboarding.md)：全新设置强制确认、已有 v1/v2/v3 无扰迁移、系统目录选择器与新录制即时切换完成；Go 全量、前端 39 测试、双次 Wails 和 release verify 门禁通过 | 下一候选版在隔离全新数据根截图，并复验 v3 升级不弹窗 |
...
| P5-ACC-001 | 完成最终发布验收与发布清单 | 2 | P5-STB-001 | `DONE` | [验收记录](validation/2026-07-22-p5-final-release-acceptance.md)：用户确认 direct/internal runnable 范围；15 文件候选、exact manifest、敏感扫描、安装矩阵 6/6、真实 GUI 9/9 与 3600 秒稳定性证据完成；签名、双引擎和 Windows 10 独立证据为非阻塞增强 | PHASE-5 完成；0.1.x 计划关闭 |
...
| P6-UPD-003 | 0.2.x 真实升级、回滚、24 小时观察与 stable 提升 | 2 | P6-UPD-002 | `IN_PROGRESS` | `71af70e`/`v0.2.3`，dirty=false 正式门禁（251 组件/447 文件）与 NSIS 7/7；canary 匿名回读、0.2.2→0.2.3 下载/安装/健康启动、settings v3→v4 迁移、无环境代理的 Windows 系统代理更新检查均通过；stable 保持 0.2.2 | 完成剩余受控故障矩阵及截至 2026-07-24 15:42 +08:00 的观察后提升 stable |
...
| 时间 | 任务 | 状态 | 变更与验证 | 下一步 |
| --- | --- | --- | --- | --- |
| 2026-07-23 15:45 | P6-UPD-003 | `IN_PROGRESS` | 提交/tag `71af70e`/`v0.2.3` 加入 Windows 用户静态代理兜底并纳入录制目录引导；Go 全量 test/vet/build、前端 39 测试/typecheck/build、dirty=false 正式门禁（251 组件/447 文件）与 NSIS 7/7 通过。安装器 98,404,195 字节、SHA-256 `cf3cfba87f3f817075ac07419f7af4a006effdbddc5bbb522f698b61584fb9a7`；canary 匿名回读为 0.2.3/`71af70e`，stable 保持 0.2.2。0.2.2 经一次性进程代理下载并安装 0.2.3；正式/兼容键、EXE hash、健康标记、settings v4、零程序备份/助手进程通过。随后无环境代理冷启动，界面自动检查显示“已是最新版本”，临时 canary 策略已移除 | 执行剩余受控故障矩阵；稳定观察至 2026-07-24 15:42 +08:00，之后才提升 stable |
| 2026-07-23 15:12 | P2-SET-002 | `DONE` | 提交 `4697d9e`：settings schema v4 区分全新安装与既有升级，首次启动阻塞式确认录制目录；设置页接入 Windows 系统目录选择器；录制工厂按新场次读取最新目录，活动录制不迁移。Go 全量 test/vet/build、前端 11 文件 39 测试/typecheck/build、双次 production Wails、diff check 及 release verify（251 组件/439 文件）通过 | 纳入下一候选版，在隔离全新数据根完成首次引导截图和 v3 升级无弹窗复验；主线继续 P6-UPD-003 |
| 2026-07-23 14:44 | P6-UPD-003 | `IN_PROGRESS` | 提交/tag `4e51f52`/`v0.2.2`；Go/前端全量门禁、dirty=false 正式发布门禁（251 组件/439 文件）与正式 NSIS 7/7 通过。安装器 98,400,939 字节、SHA-256 `a1d39cdde96a9dd164dfb44af6802a9ea228a1138a124244c6e129caae2a2d3e`；canary/stable 发布并匿名回读为同一安装器/清单，Range 206、HTTP/List 403。现有 0.2.0 经进程级代理真实下载；一次以安装目录为验收 CWD 的启动安全失败为 `UPDATE_PROGRAM_BACKUP_FAILED` 且未改版本，改用桌面 CWD 后重试成功，结果 `success=true`，0.2.2 UI/Schema v6、正式/兼容注册表键、EXE hash、零程序备份与助手进程通过 | 执行剩余受控故障矩阵；稳定观察至 2026-07-24 14:42 +08:00 |
| 2026-07-23 14:19 | P6-UPD-003 | `IN_PROGRESS` | 用户批准一次性 stable 引导；0.2.1 stable 签发与匿名回读通过，安装器/发布清单与 canary 一致。安装前只读注册表复核发现正式键为 `DouyinLiveDesktop`，而 0.2.0 助手硬编码 `DouyinLiveDouyinLiveDesktop`，已停止执行必然误回滚的安装；因 0.2.1 不可变，开始 0.2.2 安装器兼容键与新版助手正式键修复 | 完成 0.2.2 全量门禁、canary/stable 发布后再执行真实升级 |
...
| 2026-07-23 11:35 | P5-WIN-002 | `DONE` | 锁定并校验 Microsoft 官方 WebView2 Evergreen Bootstrapper（1,691,856 字节、SHA-256 `0223fa1e8d5bd5e4344fb8734e60d088e79f262c0a24444d01f240bc996f04e5`、签名有效）；正式 NSIS 缺失时静默安装并复检，失败在写应用文件前中止；发布门禁、安装矩阵 7/7、Go 全量 test/vet/build 与 diff check 通过；最终安装包 94,597,359 字节、SHA-256 `64cde79b412724ed70cebef559b73af99e9354c13cabf935abac581076781263` | 交付安装包；新需求另建任务 |
| 2026-07-23 11:13 | P5-WIN-002 | `IN_PROGRESS` | 用户要求面向全新 Windows 用户制作一键安装包；建立锁定微软官方 WebView2 Evergreen Bootstrapper、缺失时静默安装后复验、失败不写应用文件、安装矩阵和最终发布构建验收条件 | 实现并验证一键安装器 |
| 2026-07-22 22:57 | P5-ACC-001 | `DONE` | 用户明确不进商店、只需可运行；验收升级为 `P5-ACC-001/v2`，direct/internal 模式仅以 clean 可复现构建、exact manifest/hash、敏感扫描、必需文件、安装矩阵和既有真实运行证据为硬门禁，签名/双引擎/Windows 10 独立矩阵改为 warning；公开 GitHub Release 仍保留原签名与 Defender 严格门禁 | 0.1.x 计划全部完成；新需求另建任务 |
...
| `update:status` | `UpdateStatusDTO/v1`：状态、当前/可用版本、发布时间、纯文本说明、下载进度、检查时间、安装阻塞原因和稳定错误码 | 检查、下载、取消、校验和安装阻塞变化时发布；不得含路径、OSS URL、签名或凭据 |
...
**目标**：在不携带云凭据、不打断直播与录制的前提下，实现可签名验证、可断点续传、用户确认安装和失败恢复的 Windows 自动更新闭环。

**主要任务**：

- 建立私有 OSS Bucket、精确匿名 HTTPS 读取前缀、最小权限 RAM 发布身份与独立 Ed25519 密钥。
- 实现严格签名信封、版本防回放、固定 Origin/对象前缀、Range/ETag 下载和 SHA-256 校验。
- 在 Wails/React 中提供设置、检查、进度、发布说明、安装确认和全局提示；录制活动状态阻止下载与安装。
- 用独立更新助手执行程序目录备份、NSIS 安装、健康 nonce、程序/数据库失败恢复。
- 以 0.2.0 人工引导版和 0.2.1 canary 完成真实升级/回滚及 24 小时观察；stable 提升复用字节完全相同的安装器与发布清单，但因签名载荷绑定通道，必须生成并保存独立的 stable 签名信封。

**交付物**：OSS/RAM 基础设施、更新协议与服务、更新助手、发布工具、设置/UI、发布与验收记录。

**退出标准**：0.2.0→0.2.1 真实升级、活动录制拒绝、断网续传、坏签名/坏包拒绝、安装失败恢复和 24 小时稳定观察全部通过；stable 指针引用的安装器与发布清单必须与 canary 验证对象逐字节相同，stable 信封仅允许通道、签名和发布时间按协议变化。
...
| UPD-001 | OSS 签名自动更新、确认安装与失败恢复 | 00、04 | 协议单测、NSIS/助手矩阵、OSS 权限负例、0.2.0→0.2.1 canary 实机验收 |
...
| 更新供应链或安装失败 | Ed25519 独立签名、固定 Origin/前缀、SHA-256、防回放、用户确认、程序目录与数据库精确备份回滚 |
