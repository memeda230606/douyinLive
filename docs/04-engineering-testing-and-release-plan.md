...
4. 首次人工安装 0.2.0 引导版；0.2.1 先进入 canary，完成真实升级、重启、失败回滚与 24 小时稳定观察后，复用完全相同的安装器与发布清单生成 stable 信封。签名载荷绑定通道，因此 canary/stable 信封必须分别签发并保存为 `update-canary.json`/`update-stable.json`，不能声称二者字节相同。
...
- stable 提升前必须有同一 0.2.1 安装器与发布清单完成 0.2.0→0.2.1 实机升级、活动录制拒绝、断网续传、坏签名/坏包拒绝、失败恢复与 24 小时观察证据；stable 信封重新签发后须再次做严格协议与匿名回读验证。
