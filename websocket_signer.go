...
const (
	...
	tikHubSignatureAPI = "/api/v1/douyin/web/generate_wss_xb_signature"
)
...
func (s *tikhubWebsocketSigner) UpdateUserAgent(userAgent string) {
	...
	s.client.SetUserAgent(userAgent)
}
...
func (s *tikhubWebsocketSigner) Sign(ctx context.Context, roomID, userUniqueID, userAgent string) (string, error) {
	...
	resp, err := s.client.GenerateWssXbSignature(ctx, userAgent, roomID, userUniqueID)
	if err != nil {
...
//   - token: TikHub API Token。 TikHub API token.
//   - userAgent: 请求 TikHub 时使用的 User-Agent。 User-Agent used when calling TikHub.
func newTikHubClient(token, userAgent string) *tikHubClient {
	return newTikHubClientWithBaseURL(token, userAgent, tikHubBaseURL)
}

func newTikHubClientWithBaseURL(token, userAgent, baseURL string) *tikHubClient {
	client := newHTTPClient(userAgent)
	client.SetBaseURL(strings.TrimRight(strings.TrimSpace(baseURL), "/"))
	client.SetCommonBearerAuthToken(strings.TrimSpace(token))
	return &tikHubClient{httpClient: client}
}

func (c *tikHubClient) SetUserAgent(userAgent string) {
...
func (c *tikHubClient) GenerateWssXbSignature(ctx context.Context, userAgent, roomID, userUniqueID string) (*tikHubResponse, error) {
	if c == nil || c.httpClient == nil {
		return nil, errors.New("tikhub http client 未初始化")
	}
	...
	response, err := c.httpClient.R().
		...
		Get(tikHubSignatureAPI)
	if err != nil {
		return nil, err
	}

	raw := append(json.RawMessage(nil), response.Bytes()...)
	result := &tikHubResponse{
		Raw:        raw,
		StatusCode: response.GetStatusCode(),
	}
	if len(raw) > 0 {
		if err := json.Unmarshal(raw, result); err != nil {
			return nil, errors.New("tikhub 返回了无效 JSON")
		}
	}
	return result, nil
}
